...
**Execution model:** 3 fresh **sonnet-level subagents** in a 2-phase pipeline with maximum parallelism. Each agent gets clean context (no review conversation, no architectural rationale). Each reads from disk and writes to disk — no reliance on conversation memory. The pipeline survives context compaction because all intermediate state is on disk.

**Shared template:** Read `~/.claude/skills/eng-planning/templates/traceability-pipeline.md` for the full 3-agent pipeline definition with agent prompts and output formats.
...
- `{TRACE_DIR}` — working directory (see above)
- `{PRD_PATH}` — path to the approved PRD
- `{ARTIFACT_PATHS}` — paths to engineering design doc(s)
- `{CONTRACT_PATHS}` — paths to API contract(s) (if any)

**Execute the pipeline as defined in the template:**
1. **Phase 1 (parallel):** Spawn Agent 1 (Forward Tracer) and Agent 2 (Reverse Tracer) simultaneously. Each extracts what it needs from the source documents and traces in one pass.
2. **Phase 2 (sequential):** After Phase 1 completes, spawn Agent 3 (Synthesis & Verdict). Reads both trace files from disk and produces the final matrix.

**After pipeline completes:** Read `{TRACE_DIR}/traceability-matrix.md` from disk. This is the authoritative result.
...
1. **If VERDICT is PASS:** Report "PRD Traceability: 100% match confirmed (3-agent audit)" and proceed to Section 1.
